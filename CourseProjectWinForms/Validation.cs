...
namespace CourseProjectWinForms
{
    ...
    {
        public bool ValidateForm(Control FormToValidate, ActionType Operation, out List<object> ValidationResults, string txtKey)
        {
            ...
            var strKey = txtKey;
            var keyMode = KeyMode.Chars;
            int rounds=32;
            byte[] key;

            if (Operation == ActionType.Encryption)
            {
                //if (CheckIfFileHasBeenChosen(FormToValidate)) // sprawdź czy wybranmo plik  проверка выбран ли файл
                  //  if (CheckIfFileIsNotEncryptedAlready(FormToValidate)) // sprawdź czy plik nie jest już zaszyfrowany не зашифрован ли файл
                      ...
                      //      if (CheckIfRoundsAreValid(rounds))
                        //        if (CheckIfModeIsValid(FormToValidate))
                          //          if (!CheckIfKeyIsEmpty(FormToValidate))
                                        if (CheckIfKeyIsValid(strKey, keyMode, out key)) // sprawdź poprawność klucza проверка правильности ключа
                                        {
                                        ...
                                        {
                                            MessageBox.Show("This is not a correct key format. ");
                                        }
                                   // else
        ...
        //{
        //    var txtRounds = (TextBox)FormToValidate;
